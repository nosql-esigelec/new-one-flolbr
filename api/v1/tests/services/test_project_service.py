...
def test_create_project(project_service_instance):
    ...
    project_neo4j = project_service_instance.neo4j.read(
        tx_type="node",
        node_label="Project",
        properties={"project_name": "Another Test Project"}).get("result")
...
def test_delete_project(project_service_instance, project_id_fixture):
    ...
    project_neo4j = project_service_instance.neo4j.read(
        tx_type="node",
        node_label="Project",
        properties={"pid": project_id_fixture}).get("result")
...
def test_get_recommended_templates(project_service_instance):
    project_with_options_id = project_service_instance.create_project(project_with_options).get("result")
    recommended_templates = project_service_instance.get_recommended_templates(project_with_options_id).get("result")
...
def test_select_template(project_service_instance):
    ...
    template_id = project_service_instance.neo4j.read(
        tx_type="node",
        node_label="Template",
        properties={"template_name": "Cookiecutter Data Science"}).get("result")["tid"]
    relation = project_service_instance.select_template(project_with_options_id, template_id).get("result")
